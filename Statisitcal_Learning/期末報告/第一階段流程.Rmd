...
for (i in 1:283) {
  SheetList[[i]]$EPS = c(NA,E[[i]]$Diluted.EPS[-c(1,2,3)],NA)
}
```
...
###########################
###########################
index_Select = which(Ticker$category==C$.[nnn]) 

Industry_Select = SheetList[[index_Select[1]]]
Industry_Select$Firm = Ticker$company[index_Select[1]]

for (i in 2:length(index_Select)) {
  x = SheetList[[index_Select[i]]]
  x$Firm = Ticker$company[index_Select[i]]
...
```{r}
select = vip(xgb.model, num_features = 6)
col = select$data[1]
...
controls=rpart.control(maxdepth= 5)
...
Company_Select = 
  Industry_Select_new[which(!(colnames(Industry_Select_new) %in%
                                c("EPS","Firm","Dates")))]
p_C = predict(cart.model,Company_Select[,-15],type = "class")
Industry_Select_new$Firm[which(p_C==1)] %>%table() %>% as.matrix() 
```
